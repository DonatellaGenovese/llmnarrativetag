> **Vendored subset — see [llmnarrativetag](..)**
>
> This directory is a partial copy of
> [jet-universe/particle_transformer](https://github.com/jet-universe/particle_transformer)
> at commit `2925bdb`, kept here so the surrogate pipeline in `../surrogate/`
> is reproducible without a separate checkout. **Included**: network defs
> (`networks/`), dataloader, weaver configs (`data/`), train scripts,
> dataset-conversion utils (`utils/`). **Not included** (too large /
> regenerable / hosted externally): raw datasets, `.venv`, trained
> checkpoints, training run logs, and the pre-computed ParT teacher logits
> (`predict_output/`, needed by `../surrogate/` — see its README for where to
> point it). Get datasets with `get_datasets.py`; copy `env.sh.template` →
> `env.sh` and fill in local dataset paths before training/inference.
> Figures and notebooks from the upstream repo were dropped as not needed
> here.
>
> Everything below this note is the original upstream README, unmodified.
